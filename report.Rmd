...
```{r sharedplots, message=FALSE, warning=FALSE}
...
# gt.atype1: "Assigned Types By Read Alignment Characteristics, overview"
flextable(df.read1) %>%
...
# gt.atype.vector1: "Assigned AAV Types By Read Alignment Characteristics, overview"
flextable(df.read.vector1) %>%
      set_header_labels(values=c("Assigned Type",  "Count", "Frequency (%)"))
```
...
# gt.atype.vector2: "Assigned AAV Types, detailed (top 20 only)"
flextable(df.read.vector2[1:20,]) %>%
      set_header_labels(values=c("Assigned Type, detailed", "Assigned Subtype", "Count", "Frequency (%)"))
```
...
if (exists("scff") & (nrow(scff) > 1)) {
  # gt.sc.flipflop: "Flip/Flop configurations, scAAV only"
  flextable(scff) %>%
...
if (exists("ssff") & (nrow(ssff) > 1)) {
  # gt.ss.flipflop: "Flip/Flop configurations, ssAAV only"
  flextable(ssff) %>%
    set_header_labels(values=c("type", "subtype", "leftITR", "rightITR", "count"))
} else {
...
```{r lenhisto2_sc, message=FALSE, warning=FALSE}
...
```{r lenhisto2_ss, message=FALSE, warning=FALSE}
...
```{r lenhisto2_rc_start, message=FALSE, warning=FALSE}
if (dim(x.read.repcap)[1] > 10) { # only plot if at least 10 reads
...
```{r lenhisto2_rc_end, message=FALSE, warning=FALSE}
if (dim(x.read.repcap)[1] > 10) { # only plot if at least 10 reads
...
```{r lenhisto2_rc_size, message=FALSE, warning=FALSE}
if (dim(x.read.repcap)[1] > 10) { # only plot if at least 10 reads
...
```{r varplots_sub, message=FALSE, warning=FALSE}
grid.arrange(p1.err_sub)
```

## Deletions

```{r varplots_del, message=FALSE, warning=FALSE}
grid.arrange(p1.err_del)
```

## Insertions

```{r varplots_ins, message=FALSE, warning=FALSE}
